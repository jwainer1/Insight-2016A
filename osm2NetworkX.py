...
##G=read_osm("data/manhattan.osm")
##pos = []

##for n in G:
    ##print G.node[n]
    ##print "lat: ", G.node[n]['lat'], " lon: ", G.node[n]['lon']
    ##pos.append([float(G.node[n]['lat']), float(G.node[n]['lon'])])


#plt.plot([G.node[n]['lon'] for n in G], [G.node[n]['lat'] for n in G], ',')
##networkx.draw(G, pos)
